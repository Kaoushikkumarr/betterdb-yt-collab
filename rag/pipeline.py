...
from .config import get_embedder, get_openai, get_redis, get_settings
...
async def ingest_pdf(file_bytes: bytes, filename: str) -> dict:
    ...
    # Embed locally using sentence-transformers (free, no API quota)
    embedder = get_embedder()
    all_embeddings: list[list[float]] = embedder.encode(chunks, show_progress_bar=False).tolist()
...
async def query_rag(query: str, user_id: str, session_id: str) -> dict:
    ...
    # 2. Embed query locally (free, no API quota)
    embedder = get_embedder()
